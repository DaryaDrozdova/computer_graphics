...
// balancing the velocity of the camera
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "LearnOpenGL", NULL, NULL);//creating the window
    if (window == NULL)
    {
    ...
    glEnable(GL_DEPTH_TEST);  //enabling depth test to draw right sides
    glEnable(GL_BLEND);  //semi-transparent windows
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);  //alpha of the sourse color vector for the source factor and (1-alpha) for the destination vector
    ...
    {
        float currentFrame = glfwGetTime();//calculating deltaTime each iteration
...
void processInput(GLFWwindow* window)//checks if the key is pressed
{
    ...
    if (glfwGetKey(window, GLFW_KEY_F) == GLFW_PRESS && !effectKeyPressed)
    {
        effect = !effect;
        effectKeyPressed = true;
    }
